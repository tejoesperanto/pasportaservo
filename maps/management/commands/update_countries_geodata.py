...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if (
            not update_countries
            or update_countries == "ALL"
            or "ALL" in update_countries
        ):
        ...
        for country_code, i in [
            (k, i)
            for i, k in enumerate(COUNTRIES.keys(), start=1)
            ...
            if not georesult:
                georesult = geocode(
                    ...
                    country=autonomous_state_codes.get(
                        country_code, political_country_code
                    ),
                    multiple=True,
                )
            ...
            for j, res in enumerate(georesult):
                if (
                    country_code not in autonomous_state_codes
                    and obj_type(res) == "country"
                ) or (
                    country_code in autonomous_state_codes and obj_type(res) == "state"
                ):
            ...
            if (
                len(georesult) == 0
                or pointed_result_index is None
                or not georesult.bbox
            ):
                georesult = geocode(str(COUNTRIES[country_code]), multiple=True)
                if (
                    len(georesult)
                    and obj_type(georesult) != "country"
                    and country_code != "AQ"
                ):
                    for j, res in enumerate(georesult):
                        if (
                            obj_type(res) in ("country", "state", "county", "island")
                            and res._components.get("ISO_3166-1_alpha-2")
            ...
            if self.verbosity >= 3:
                if (
                    country_code in code_replacements
                    or country_code in name_replacements
                ):
                    self.stdout.write(
                        "\t{code} ({name})".format(
                ...
                if (
                    obj_type(georesult) == "country"
                    or obj_type(georesult) == "state"
                    and country_code in autonomous_state_codes
                ):
                ...
                self.stdout.write(
                    style("\t--  {components}".format(components=georesult._components))
                )
                if general_result_index is not None:
                    self.stdout.write(
                        "\t--  [result {index} out of the returned general list]".format(
                            index=general_result_index + 1
                        )
                    )
                self.stdout.write(
                    "\t--  {bbox}, center: {center}".format(
        ...
        file_new_contents += json.dumps(country_list, indent=4).replace(
            "null", str(None)
        )
        ...
        if self.verbosity >= 1:
            if not update_failed:
                self.stdout.write(
                    self.style.SUCCESS(
                        "** Geodata update successfully completed. {} countries updated.".format(
                            len(update_succeeded)
                        )
                    )
                )
                if update_succeeded and self.verbosity >= 2:
                    self.stdout.write(
                        self.style.SUCCESS("   {}".format(", ".join(update_succeeded)))
                    )
            else:
                self.stdout.write(
                    self.style.ERROR(
...
def set_coord(geo_result_to, geo_result_from, angle, index=None):
    ...
    pair = (
        geo_result_from.bbox[translate[angle]] if geo_result_from.ok else [None, None]
    )
...
def find_and_set_coord(
    geo_result, region, country_code, condition, angle, index=None, update_center=False
):
    """
    ...
    try:
        coords = next(
            filter(
                ...
                else lambda r: obj_type(r) == condition,
...
def custom_calculation(country_code):
    ...
    class GeoResultStub(object):
        ...
        def __str__(self):
            return "<[{}] Opencage - Manual Geocoding>".format(
                "OK" if not self.error else "ERROR"
            )
    ...
    if country_code == "AU":
        ...
        find_and_set_coord(
            result, "Tasmania", "AU", lambda r: "island" in r._components, "sw", 1
        )
    ...
    if country_code == "CL":
        ...
        set_coord(
            result,
            geocode("Región Aysén del General Carlos Ibáñez del Campo", country="CL"),
            "sw",
            0,
        )
        set_coord(
            result, geocode("Región de Arica y Parinacota", country="CL"), "ne", 1
        )
    ...
    if country_code == "EC":
        ...
        result.xy = [
            round(
                (
                    (result.bbox["northeast"][0] or 0)
                    + (result.bbox["southwest"][0] or 0)
                )
                / 2,
                7,
            ),
            round(
                (
                    (result.bbox["northeast"][1] or 0)
                    + (result.bbox["southwest"][1] or 0)
                )
                / 2,
                7,
            ),
    ...
    if country_code == "PT":
        ...
        set_coord(
            result, geocode("Área Metropolitana de Lisboa", country="PT"), "sw", 0
        )
    ...
    if country_code == "SG":
        ...
        set_coord(
            result,
            geocode("South East Community Development Council", country="SG"),
            "ne",
            0,
        )
        set_coord(
            result,
            geocode("North West Community Development Council", country="SG"),
            "ne",
            1,
        )
        set_coord(
            result,
            geocode("South West Community Development Council", country="SG"),
            "sw",
        )
    ...
    if country_code == "TV":
        ...
        result.bbox = {
            "northeast": [179.871061, -5.642230],
            "southwest": [176.078332, -10.791658],
        }
    ...
    if (
        ...
        and any(
            point is None
            for point in result.bbox["northeast"] + result.bbox["southwest"] + result.xy
        )
    ):
