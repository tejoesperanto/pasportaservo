...
class EndpointsView(generic.View):
    def get(self, request, *args, **kwargs):
        ...
        if type == "world":
            endpoints.update(
                {
                    "world_map_style": reverse_lazy(
                        "map_style", kwargs={"style": "positron"}
                    ),
        ...
        if type == "region":
            ...
            endpoints.update(
                {
                    "region_map_style": reverse_lazy(
                        "map_style", kwargs={"style": "klokantech"}
                    ),
                    "region_map_data": reverse_lazy(
                        "country_map_data", kwargs=region_kwargs
                    ),
                }
            )
        if type == "place":
...
class PublicDataView(GeoJSONLayerView):
    ...
    def dispatch(self, request, *args, **kwargs):
        request.META["HTTP_X_USER_STATUS"] = (
            "Authenticated" if request.user.is_authenticated else "Anonymous"
        )
    ...
    def get_queryset(self):
        ...
        return (
            PlottablePlace.objects_raw.filter(available=True)
            .exclude(
...
class CountryDataView(AuthMixin, GeoJSONLayerView):
    ...
    def get_queryset(self):
        queryset = PlottablePlace.available_objects.filter(
        ...
        if self.in_book_status is not None:
            narrowing_func = getattr(
                queryset, "filter" if self.in_book_status else "exclude"
            )
            queryset = narrowing_func(in_book=True, visibility__visible_in_book=True)
        return (
            queryset.exclude(Q(location__isnull=True) | Q(location=Point([])))
            .select_related("owner")
            .defer("address", "description", "short_description", "owner__description")
        )
