...
class VisibilityForm(forms.ModelForm):
        ...
        for venue in self.venues():
            ...
            if venue.venue_name.startswith("online_"):
                attrs.update(
                    {
                        "data-tied": str(
                            self.obj.visibility.rules.get("tied_online", False)
                        )
                    }
                )
    ...
    def venues(self, restrict_to=""):
        ...
        for f in self.fields:
            if f.startswith(self._meta.model._PREFIX + restrict_to):
                bound_field = self[f]
                bound_field.field_name, bound_field.venue_name = (
                    f,
                    f[len(self._meta.model._PREFIX) :],
                )
                yield bound_field
    ...
    def obj(self):
        ...
        if self.is_bound and self.instance.model_type == self.instance.DEFAULT_TYPE:
            raise ValueError(
                ...
                "for key {pk} and {profile!r}. This most likely indicates tampering.".format(
                    pk=self.requested_pk, profile=self.profile
                )
            )
        wrappers = {
        ...
        return wrappers.get(self.instance.model_type, lambda c: c)(
    ...
    class FamilyMemberAsset:
        ...
        def icon(self):
            template = (
                '<span class="fa ps-users" title="{title}" '
    ...
    def save(self, commit=True):
        ...
        counterparty = {
            "online_public": "online_authed",
            "online_authed": "online_public",
            None: "",
        }[venue]
        ripple = [
            ...
            (not value)
            and venue == "online_public"
            and visibility.rules.get("tied_online"),
        ]
        if any(ripple):
...
class VisibilityFormSetBase(forms.BaseModelFormSet):
    ...
    def __init__(self, *args, **kwargs):
        ...
        PLACE, FAMILY_MEMBERS, PHONE, PUBLIC_EMAIL = (
            vis.type()
            for vis in [
                VisibilitySettingsForPlace,
                VisibilitySettingsForFamilyMembers,
        ...
        what = Q()
        owned_places = self.profile.owned_places.exclude(deleted=True).prefetch_related(
            "family_members"
        )
        what |= Q(model_type=PLACE, place__in=owned_places)
        what |= Q(
            model_type=FAMILY_MEMBERS,
            family_members__in=[
                place.pk
        ...
        what |= (
            Q(model_type=PUBLIC_EMAIL, profile=self.profile)
    ...
    def get_form_kwargs(self, index):
        ...
        if index >= len(self.queryset):
            raise IndexError(
                "Form #{id} does not exist in the queryset, for {profile!r}. "
                "This most likely indicates tampering.".format(
                    id=index, profile=self.profile
                )
            )
