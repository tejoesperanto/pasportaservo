    ...
    def supervisors(self, obj):
        def get_formatted_list():
            for u in obj.user_set.all():
                ...
                account_link = '<a href="{url}">{username}</a>'.format(
                    url=link, username=u
                )
                ...
                indicator = (
                    '<span style="color:#dd4646">&#x2718;!</span>' if is_deleted else ""
                )
                yield " ".join([indicator, account_link, profile_link])

        return (
            format_html(", ".join(get_formatted_list())) if len(obj.name) == 2 else "-"
        )
    ...
    class CountryGroup(Group):
        ...
        def __str__(self):
            ...
            return format_html(
                ...
                country_name=Country(self.name).name,
            )
...
class AgreementAdmin(admin.ModelAdmin):
    ...
    def policy_link(self, obj):
        ...
        try:
            policy = FlatPage.objects.values_list("id", flat=True).get(
                url="/privacy-policy-{}/".format(obj.policy_version)
            )
            link = reverse("admin:flatpages_flatpage_change", args=[policy])
            value = format_html(
                '<a href="{url}">{policy}</a>', url=link, policy=obj.policy_version
            )
...
class TrackingModelAdmin(ShowConfirmedMixin):
    ...
    def get_field_queryset(self, db, db_field, request):
        if db_field.name == "checked_by":
            return (
                self.InstanceApprover.objects.filter(
                    Q(is_superuser=True) | Q(groups__name__regex=r"[A-Z]{2}")
                )
                .distinct()
                .select_related("profile")
                .defer("profile__description")
                .order_by("username")
            )
        return super().get_field_queryset(db, db_field, request)
...
class VisibilityAdmin(admin.ModelAdmin):
    visibility_fields = tuple(
        f.name
        for f in VisibilitySettings._meta.get_fields()
...
class ProfileAdmin(TrackingModelAdmin, ShowDeletedMixin, admin.ModelAdmin):
    ...
    def user_link(self, obj):
        try:
            link = reverse("admin:auth_user_change", args=[obj.user.pk])
            return format_html(
                '<a href="{url}">{username}</a>', url=link, username=obj.user
            )
        except AttributeError:
            return "-"
    ...
    def supervisor(self, obj):
        country_list = CustomGroupAdmin.CountryGroup.objects.filter(
            name__regex=r"^[A-Z]{2}$", user__pk=obj.user_id if obj.user_id else -1
        )
        if country_list:
...
class PlaceAdmin(
    TrackingModelAdmin, ShowCountryMixin, ShowDeletedMixin, admin.ModelAdmin
):
    list_display = (
        "id",
        "city",
    ...
    def owner_link(self, obj):
        return format_html(
            '<a href="{url}">{name}</a>', url=obj.owner.get_admin_url(), name=obj.owner
        )
...
class CountryRegionAdmin(ShowCountryMixin, admin.ModelAdmin):
    ...
    list_filter = (CountryMentionedOnlyFilter,)
...
class ConditionAdmin(admin.ModelAdmin):
    ...
    def icon(self, obj):
        return format_html(
            '<img src="{static}img/cond_{slug}.svg" style="width:4ex; height:4ex;"/>',
