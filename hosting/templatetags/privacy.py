    ...
    def render(self, context):
        ...
        if self.attribute and not subvar:
            raise AttributeError(
                "'{}' did not resolve to any name of attribute on {}".format(
                    self.attribute, var._meta.model.__name__
                )
            )
        is_privileged = bool(self.privilege_context.resolve(context))

        visibility = getattr(
            var, ("{}_".format(subvar) if subvar else "") + "visibility"
        )
        result = visibility.visible_online_public or (
            is_privileged and visibility.visible_online_authed
        )
        privacy_log.debug(
            "%r%s\r (OP:%r | C:%r & OA:%r) = %r",
            var,
            ".%s" % subvar if subvar else "",
            visibility.visible_online_public,
            is_privileged,
            visibility.visible_online_authed,
            result,
        )
        if self.as_var:
            output_var = "shall_display_{}".format(
                (subvar if subvar else var._meta.model.__name__).lower()
            )
