...
class PlaceStaffListView(AuthMixin, PlaceListView):
    ...
    def get_queryset(self):
        self.base_qs = self.model.available_objects.filter(
        ...
        self.base_qs = self.base_qs.annotate(
            ...
            location_inaccurate=Case(
                When(
                    Q(location_valid=True)
                    & Q(location_confidence__lt=LocationConfidence.ACCEPTABLE),
                    then=True,
                ),
                default=False,
                output_field=BooleanField(),
            ...
            location_accurate=Case(
                When(
                    Q(location_valid=True)
                    & Q(location_confidence__gte=LocationConfidence.ACCEPTABLE),
                    then=True,
                ),
                default=False,
                output_field=BooleanField(),
        ...
        if self.in_book_status is not None:
            narrowing_func = getattr(
                self.base_qs, "filter" if self.in_book_status else "exclude"
            )
        ...
        return qs.prefetch_related("owner__user", "owner__phones").order_by(
            "-confirmed", "checked", "owner__last_name"
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["not_in_book_count"] = self.base_qs.filter(
            ~in_book_status_filter
        ).count()
        if self.in_book_status is True:
            ...
            book_filter = Q()
            context["place_count"] = (
                context["in_book_count"] + context["not_in_book_count"]
            )
        context["checked_count"] = self.base_qs.filter(
            book_filter, checked=True
        ).count()
        context["confirmed_count"] = self.base_qs.filter(
            book_filter, confirmed=True
        ).count()
        context["not_confirmed_count"] = (
            context["place_count"] - context["confirmed_count"]
        )
...
class SearchView(PlacePaginatedListView):
    queryset = Place.available_objects.filter(
        visibility__visible_online_public=True, owner__death_date__isnull=True
    )
    ...
    def get_queryset(self):
        most_recent_moniker, most_recent = (
            pgettext("value::plural", "MOST RECENT"),
            False,
        )
        ...
        qs = self.queryset.select_related("owner", "owner__user").defer(
            "address", "description", "owner__description"
        )
        ...
        if self.query and self.result.point:
            if any(
                ...
                return qs.annotate(
                    distance=Distance("location", self.result.point)
                ).order_by("distance")
                ...
                return qs.filter(country=self.result.country_code.upper()).order_by(
                    "-owner__user__last_login", "-id"
                )
        position = geocoder.ip(
        ...
        if position.point and not most_recent:
            ...
            qs = qs.annotate(
                internal_distance=Distance("location", position.point)
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        if (
            getattr(self, "country_search", False)
            and hasattr(self, "result")
            and self.result.country_code
        ):
            context["country_advisories"] = TravelAdvice.get_for_country(
                self.result.country_code.upper()
            )
        return context
