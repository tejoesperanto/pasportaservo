...
class PlaceLocationUpdateView(UpdateMixin, AuthMixin, PlaceMixin, generic.UpdateView):
    ...
    def get_success_url(self, *args, **kwargs):
        success_url = reverse_lazy(
            "place_detail_verbose", kwargs={"pk": self.object.pk}
        )
...
class PlaceDeleteView(
    DeleteMixin, AuthMixin, PlaceMixin, ProfileModifyMixin, generic.DeleteView
):
    pass
...
class PlaceDetailView(AuthMixin, PlaceMixin, generic.DetailView):
    ...
    def get_queryset(self):
        related = [
            ...
            "owner__user",
            "visibility",
            "family_members_visibility",
            "owner__email_visibility",
        ]
    ...
    def get_context_data(self, **kwargs):
        ...
        context["owner_phones"] = self.object.owner.phones.filter(
            deleted=False
        ...
        if (location is None or location.empty) and is_authenticated:
            ...
            try:
                ...
                georegions = Whereabouts.objects.filter(
                    ...
                    state=place.state_province.upper(),
        ...
        if location is None and bounds is None:
            ...
            if place.location and not place.location.empty and is_authenticated:
                ...
                bounds = [
                    ...
                    {
                        "geom": LineString(
                            coords["bbox"]["southwest"],
                            coords["bbox"]["northeast"],
                            srid=SRID,
                        )
                    },
                ]
    ...
    def calculate_blocking(place):
        ...
        if place.is_blocked:
            ...
            if place.blocked_from and place.blocked_from > today:
                ...
                block["format_from"] = (
                    "MONTH_DAY_FORMAT"
                    if place.blocked_from.year == today.year
                    else "DATE_FORMAT"
                )
            if place.blocked_until and place.blocked_until >= today:
                ...
                block["format_until"] = (
                    "MONTH_DAY_FORMAT"
                    if place.blocked_until.year == today.year
                    else "DATE_FORMAT"
                )
        else:
            block["enabled"] = False
    ...
    def validate_access(self):
        ...
        result = namedtuple(
            "AccessConstraint",
            "redirect, is_authorized, is_supervisor, is_family_member",
        )
        ...
        if not user.is_authenticated:
            ...
            if any(cases):
                ...
                self._access_validated = result(
                    self.handle_no_permission(), None, None, None
                )
                return self._access_validated

        is_authorized = user in place.authorized_users_cache(
            also_deleted=True, complete=False
        )
        is_supervisor = self.role >= SUPERVISOR
        is_family_member = (
            getattr(user, "profile", None) in place.family_members_cache()
        )
        ...
        self._access_validated = result(
            content_unavailable, is_authorized, is_supervisor, is_family_member
        )
        return self._access_validated
        ...
        if not self.verbose_view:
            ...
            if not any(cases):
                return HttpResponseRedirect(
                    reverse_lazy("place_detail", kwargs={"pk": self.kwargs["pk"]})
                )
...
class UserAuthorizeView(AuthMixin, generic.FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if valid_back_link:
            context["back_to"] = (
                m.group(1).lower() if not m.group(2) else m.group(2).lower()
            )

        def order_by_name(user):
            try:
                return (
                    " ".join((user.profile.first_name, user.profile.last_name)).strip()
                    or user.username
                ).lower()
        ...
        context["authorized_set"] = [
            ...
            for user in sorted(
                self.place.authorized_users_cache(also_deleted=True), key=order_by_name
            )
        ]
        return context
    ...
    def get_success_url(self):
        ...
        if redirect_to:
            return format_lazy(
                "{}?{}", success_url, next_link(self.request, redirect_to)
            )
        return success_url
