...
class PhoneUpdateView(
    UpdateMixin, AuthMixin, PhoneMixin, ProfileModifyMixin, generic.UpdateView
):
...
class PhoneDeleteView(
    DeleteMixin, AuthMixin, PhoneMixin, ProfileModifyMixin, generic.DeleteView
):
    pass
...
class PhonePriorityChangeView(AuthMixin, ProfileMixin, generic.View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        phone_ids = re.findall(
            r"^t(\d+)$", "\n".join(request.POST.getlist("onr")), re.MULTILINE
        )
