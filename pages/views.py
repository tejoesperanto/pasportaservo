...
class AboutView(generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        all_places = Place.available_objects.filter(
    ...
    def get_countries(self, filter_for_book=False, filter_for_supervisor=False):
        ...
        places = Place.available_objects.filter(
            ...
            if filter_for_book
                ...
                (book_filter | online_filter)
                if filter_for_supervisor
                else online_filter
            )
        )
        ...
        for country in countries:
            try:
                ...
                country.supervisors = sorted(
                    user.profile
                    for user in group.user_set.filter(
                        is_active=True,
                        profile__isnull=False,
                        profile__deleted_on__isnull=True,
                    )
                )
