...
ADMINS = (("Pasporta Servo", "saluton@pasportaservo.org"),)
try:
    ...
    ADMINS += tuple(
        cleanup(*("§ " + admin).rsplit(maxsplit=1))
...
def get_current_commit():
    ...
    branch = (
        refnames.strip().replace("HEAD -> ", "").replace("HEAD, ", "").split(",")[0]
    )
...
POSTMAN_NOTIFICATION_APPROVAL = lambda user, *args: not user.email.startswith(
    INVALID_PREFIX
)
POSTMAN_NOTIFIER_APP = None

MAPBOX_GL_BASE_STATIC = (
    "https://api.tiles.mapbox.com/mapbox-gl-js/v1.6.1/mapbox-gl.{ext}"
)
