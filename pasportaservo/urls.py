...
url_index_postman = "/".join(
    [reverse_lazy("postman:inbox").rstrip("/").rsplit("/", maxsplit=1)[0], ""]
)
...
for module in (m for m in urlpatterns if m.app_name == "postman"):
    for pattern in (
        p
        for p in module.url_patterns
        if p.name in ["write", "reply", "view", "view_conversation"]
    ):
