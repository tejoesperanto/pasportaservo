            ...
            auth_stats["context_role"].update(
                {
                    "role": view.role,
                    "role_name": next(
                        (name for name, val in auth_roles.items() if val == view.role),
                        None,
                    ),
                    "role_required": "= " + str(view.exact_role)
                    if getattr(view, "exact_role", None)
                    else ">= " + str(view.minimum_role),
            ...
            auth_stats["context_role"].update(
                {
                    ...
                    "perms": supervisor_of(request.user)
...
class CustomLoggingPanel(LoggingPanel):
    def generate_stats(self, request, response):
        ...
        for rec in records:
            ...
            msg = "".join(
                map(
                    lambda t: t.replace("\t", "<div style='margin-left:1.5em'>")
                    + "</div>" * t.count("\t"),
                    msg,
                )
            )
