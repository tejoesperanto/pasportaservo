...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        country_regions = CountryRegion.objects.filter(country=country_code).values(
            "iso_code"
        )
        region_is_mandatory = "administrativeArea" in COUNTRIES_DATA[country_code].get(
            "required_fields", []
        )
        if country_regions:
            ...
            places = Place.all_objects.filter(country=country_code).exclude(
                valid_address
            )
        else:
                ...
                sorted(
                    [
                        ...
                        for (
                            ...
                            latin_code,
                            latin_name,
                        ) in country_regions.values_list(
