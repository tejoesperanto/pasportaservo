...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.print_overall_summary(
            country_result, (False, None), make_style(fg="green"), "** Update skipped"
        )
        self.print_overall_summary(
            country_result,
            (True, False),
        ...
        self.print_overall_summary(
            country_result, (False, False), self.style.ERROR, "** Update failed"
        )
        if any(country_removals.values()):
            if self.verbosity >= 1:
                self.stdout.write(
                    make_style(fg="red")(
                        "** Some regions of {} were removed!".format(
                            ", ".join(country_removals.keys())
                        )
                    ),
                    ending=" ",
                )
                self.stdout.write(
                    ...
                    "addresses in the database.".format(
                        make_style(opts=("underscore",))("check_country_regions")
                    )
                )
            if self.verbosity == 2:
                self.stdout.write(
    ...
    def get_country_regions_queryset(self, country_code):
        return CountryRegion.objects.filter(country=country_code).values(
            "iso_code", "latin_code", "latin_name"
        )
    ...
    def post_process(self, country_code, all_iso_codes):
        ...
        self.removed_regions = [
            r
            for c, r in self.old_regions.items()
    ...
    def load_commerceguys_data(self, country_code):
        try:
            ...
            try:
                json_data = (
                    json.loads(json_data.text) if json_data.status_code == 200 else None
                )
    ...
    def process_commerceguys_data(self, country_code, json_data):
        ...
        for i, (region_key, region) in enumerate(
            json_data["subdivisions"].items(), start=1
        ):
            if not region:
                region = {}
    ...
    def process_geonames_data(self, country_code, geonames_data):
        code_key = {"ADM1": "adminCodes1", "ADM2": "adminCodes2"}[
        ...
        for i in range(len(regions)):
            ...
            update_fields = dict(
                latin_code=self.normalize_toponym(
                    region["toponymName"], toponym_trim_re
                ),
            )
            ...
            CountryRegion.objects.update_or_create(
                country=country_code, iso_code=region_iso_code, defaults=update_fields
            )
            iso_codes.append(region_iso_code)
    ...
    def get_localised_name(self, region_data, for_lang):
        ...
        try:
            localised_name = next(
                filter(lambda name: "isPreferredName" in name, alternate_names)
            )
    ...
    def update_esperanto_name_and_bbox(self, country_code, json_data, geonames_data):
        all_iso_codes = CountryRegion.objects.filter(country=country_code).values_list(
            "iso_code", "latin_code", "latin_name", "esperanto_name"
        )
        code_key = {"ADM1": "adminCodes1", "ADM2": "adminCodes2"}[
            COUNTRIES_DATA[country_code]["administrative_area_code"]
        ]
        ...
        for region_code, latin_code, latin_name, existing_eoname in all_iso_codes:
            ...
            if "bbox" in region:
                Whereabouts.objects.update_or_create(
                    ...
                    defaults=dict(
                        ...
                        center=Point(
                            float(region["lng"]), float(region["lat"]), srid=SRID
                        ),
                    ),
                )
            ...
            if region_eoname != existing_eoname:
                (
                    CountryRegion.objects.filter(
                        country=country_code, iso_code=region_code
                    ).update(esperanto_name=region_eoname)
                )
                updated_eonames[
                    region_code
                ] = f"{existing_eoname or '.'}->{region_eoname}"
        ...
        if self.verbosity >= 3:
            if updated_eonames:
                self.stdout.write(
                    make_style(fg="cyan")(
                        ("\n" if linebreak else "")
                        + "\tUPDATED: {}".format(updated_eonames)
                    )
                )

    def print_country_summary(self, country_code, region_names, source):
        if self.verbosity >= 2:
            self.stdout.write("\t" + str(list(region_names)) + f"   from {source}")
        if self.verbosity >= 3:
                ...
                self.stdout.write(
                    make_style(fg="green")(
                        "\tADDED:   {}".format(
                            [
                                tuple(r.values())
                                for c, r in self.new_regions.items()
                                if c in added
                            ]
                        )
                    )
            ...
            changed = [
                ...
                if any(
                    self.old_regions[c][field] != self.new_regions[c][field]
                    for field in self.old_regions[c].keys()
                )
            ]
            if changed:
                self.stdout.write(
                    make_style(fg="magenta")("\tCHANGED: {}".format(changed))
                )

    def print_overall_summary(
        self, country_result, expected_status, status_style, status_message
    ):
        if self.verbosity >= 1:
            status_countries = sorted(
                list(
                    filter(
                        lambda c: country_result[c] == expected_status, country_result
                    )
                )
            )
            if status_countries:
