...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if self.verbosity >= 1:
                self.stdout.write(
                    self.style.ERROR(f"** Metadata update failed. {e.args[0].reason}.")
                )
            sys.exit(1)
            ...
            json_data = re.search(
                r"\$definitions = \[(.+),\s+\];", json_data.text, re.DOTALL
            ).group(1)
        ...
        for i, country in [
            (i, k)
            for i, k in enumerate(csv_data, start=1)
            if k["#ISO"] in COUNTRIES_DATA
        ]:
            code = country["#ISO"]
            country_list[code]["population"] = int(country["Population"])
            if country["Languages"]:
                country_list[code]["languages"] = (
                    country["Languages"].rstrip(",").split(",")
                )
            ...
            if not country_list[code].get("postcode_regex"):
                country_list[code]["postcode_regex"] = (
                    country["Postal Code Regex"].lstrip("^").rstrip("$")
                )
            ...
            if (
                not country_list[code]["postcode_format"]
                and country_list[code]["postcode_regex"]
            ):
            ...
            if self.verbosity >= 2:
                ...
                if not postcode_pattern and country["Postal Code Regex"]:
                    self.stdout.write(
                        f"\tpostcode regex from geonames: {country['Postal Code Regex']}"
                    )
        ...
        file_new_contents += json.dumps(
            country_list, indent=4, ensure_ascii=False
        ).replace("null", str(None))
        ...
        if self.verbosity >= 1:
            self.stdout.write(
                self.style.SUCCESS("** Metadata update successfully completed.")
            )
