...
class SupervisorAuthBackend(ModelBackend):
    ...
    _perm_sv_particular_re = re.compile(
        r"^%s\.[A-Z]{2}$" % PERM_SUPERVISOR.replace(".", "\\."), re.I
    )
    ...
    def get_user_supervisor_of(self, user_obj, obj=None, code=False):
        ...
        if not hasattr(user_obj, cache_name):
            ...
            user_countries = frozenset(
                Country(g.name) for g in user_groups if len(g.name) == 2
            )
        ...
        if obj is not None:
            if isinstance(obj, Country):
                ...
                countries = obj.owned_places.filter(deleted=False).values_list(
                    "country", flat=True
                )
    ...
    def has_perm(self, user_obj, perm, obj=None):
        ...
        if auth_log.getEffectiveLevel() == logging.DEBUG:
            auth_log.debug(
                ...
                perm,
                user_obj,
                "%s %s" % ("object", repr(obj)) if obj else "any records",
            )
    ...
    def get_group_permissions(self, user_obj, obj=None):
        ...
        if any(groups):
            ...
            if auth_log.getEffectiveLevel() == logging.DEBUG:
                auth_log.debug(
                    "\tUser's group perms:  %s", set(getattr(user_obj, cache_name))
                )
            if obj is None:
                perms.update(getattr(user_obj, cache_name))
            else:
                groups_for_obj = set(
                    self.get_user_supervisor_of(user_obj, obj, code=True)
                )
                perms_for_obj = set(
                    "%s.%s" % (PERM_SUPERVISOR, g) for g in groups_for_obj
                )
...
class AuthMixin(AccessMixin):
    ...
    permission_denied_message = _(
        "Only the supervisors of {this_country} can access this page"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        def _dispatch_wrapper(
            wrapped_self, wrapped_request, *wrapped_args, **wrapped_kwargs
        ):
            result = dispatch_func(wrapped_request, *wrapped_args, **wrapped_kwargs)
            if not hasattr(wrapped_self, "role") and not isinstance(
                result, HttpResponseNotAllowed
            ):
                warnings.warn(
                    "AuthMixin is present on the view {} but no authorization check was performed. "
                    "Check super() calls and order of inheritance.".format(
                        self.__class__.__name__
                    ),
                    AuthMixin.MisconfigurationWarning,
                    stacklevel=2,
                )
            return result
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        """
        if (
            getattr(self, "exact_role", None) == ANONYMOUS
            or self.minimum_role == ANONYMOUS
        ):
    ...
    def _auth_verify(self, object, context_omitted=False):
        ...
        if getattr(self, "exact_role", None):
            roles_allowed = (
                self.exact_role
                if isinstance(self.exact_role, tuple)
                else (self.exact_role,)
            )
            auth_log.info(
                "exact role allowed: {- %s -} , current role: {- %s -}",
                " or ".join(str(r) for r in roles_allowed),
                self.role,
            )
        ...
        elif self.display_permission_denied and self.request.user.has_perm(
            PERM_SUPERVISOR
        ):
            raise PermissionDenied(
                self.get_permission_denied_message(object, context_omitted), self
            )
        else:
    ...
    def get_permission_denied_message(self, object, context_omitted=False):
        ...
        return format_lazy(
            ...
            this_country=join_lazy(
                ", ", countries, lambda item: str(Country(item).name)
            ),
        )
