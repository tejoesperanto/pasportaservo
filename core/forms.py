...
class UserRegistrationForm(
    UsernameFormMixin, PasswordFormMixin, SystemEmailFormMixin, UserCreationForm
):
    # Honeypot:
    ...
    def __init__(self, *args, **kwargs):
        ...
        for fieldname in ["username", "password1", "password2", "email"]:
            ...
            self.fields[fieldname].widget.attrs["placeholder"] = self.fields[
                fieldname
            ].label
...
class UserAuthenticationForm(AuthenticationForm):
    ...
    def confirm_login_allowed(self, user):
        ...
        if not user.is_active:
            ...
            auth_log.warning(
                (
                    self.admin_inactive_user_notification
                    + ", but the account is deactivated [{cid}]."
                ).format(u=user, cid=case_id)
            )
            self.request.session["restore_request_id"] = (
                case_id,
            ...
            raise forms.ValidationError(
                [
                    ...
                    forms.ValidationError(
                        _(
                            'Would you like to re-enable it? <a href="%(url)s">Inform an administrator.</a>'
                        ),
                        code="restore_hint",
...
class SystemPasswordResetRequestForm(PasswordResetForm):
    admin_inactive_user_notification = (
        "User '{u.username}' tried to reset the login password"
    )
    ...
    def get_users(self, email):
        ...
        lookup_users = users.filter(
            Q(email__iexact=email) | Q(email__iexact=invalid_email)
        )
        ...
        return map(
            ...
            (
                ...
                and not u.password.startswith(UNUSABLE_PASSWORD_PREFIX)
            ),
        )
...
class UsernameRemindRequestForm(SystemPasswordResetRequestForm):
    admin_inactive_user_notification = (
        "User '{u.username}' requested a reminder of the username"
    )


class MassMailForm(forms.Form):
    heading = forms.CharField(label=_("Heading"), initial=_("Announcement"))
    ...
    preheader = forms.CharField(
        label=_("Preheader"), max_length=100, widget=forms.Textarea(attrs={"rows": 2})
    )
