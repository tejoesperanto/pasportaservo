...
class AccountFlagsMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        if "flag_analytics_setup" not in request.session:
            ...
            pref = Preferences.objects.filter(
                profile=profile, site_analytics_consent__isnull=True
            )
        ...
        try:
            ...
            if hasattr(
                trouble_view.func, "view_class"
            ) and trouble_view.func.view_class not in [
                LoginView,
                LogoutView,
                HomeView,
                AgreementRejectView,
        ...
        if trouble_view is not None and len(birth_date) != 0 and birth_date[0]:
            ...
            try:
                ...
                raise PermissionDenied(
                    format_lazy(
                        _(
                            "Unfortunately, you are still too young to use Pasporta Servo. "
        ...
        if trouble_view is not None:
            agreement = Agreement.objects.filter(
                user=request.user,
                policy_version__in=policy.values_list("version"),
        ...
        if (
            request.path.startswith(url_index_postman)
            and len(birth_date) == 0
            ...
            t = TemplateResponse(
                request,
                "registration/profile_create.html",
                status=403,
                context={
