...
class PostModelTests(WebTest):
    def test_field_max_lengths(self):
        post = PostFactory.build()
        self.assertEquals(post._meta.get_field('title').max_length, 200)
    ...
    def test_absolute_url(self):
        post = PostFactory()
        self.assertEquals(post.get_absolute_url(), '/blogo/{}/'.format(post.slug))
...
class PublishedManagerTests(WebTest):
    ...
    def test_published(self):
        ...
        # self.assertEquals(len(mgr.published()), 50)
        for n in (10, 6, 3):
    ...
    def test_published_with_no_posts(self):
        mgr = Post.objects
        self.assertEquals(len(mgr.published()), 0)
