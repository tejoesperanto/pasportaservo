...
class ProfileFormTestingBase:
    ...
    def setUpTestData(cls):
        ...
        cls.profile_with_no_places_deceased = TaggedProfile(
            ProfileFactory(deceased=True), "deceased"
        )
    ...
    def test_init(self):
        form_empty = self._init_form(
            empty=True, user=self.profile_with_no_places.obj.user
        )
        expected_fields = [
            "title",
            "first_name",
        ...
        for form in (
            form_empty,
            self._init_form(instance=self.profile_with_no_places.obj),
        ):
            for field in set(self.host_required_fields) | set(
                self.book_required_fields
            ):
    ...
    def test_invalid_birth_date_in_future_or_past(self):
        ...
        for birth_date, violation_type in (
            [today + timedelta(days=1), "too unborn"],
            [max_past - timedelta(days=1), "too dead"],
        ):
            for profile, profile_tag in (
                self.profile_with_no_places,
                ...
                with self.subTest(
                    ...
                    if violation_type == "too unborn":
                        if profile_tag == "simple" or type(form) is ProfileCreateForm:
                            self.assertEqual(
                                ...
                                {
                                    "birth_date": [
                                        "Ensure this value is less than or equal to {}.".format(
                                            today
                                        )
                                    ],
                                },
                            ...
                            self.assertEqual(
                                ...
                                {
                                    "birth_date": [
                                        "The indicated date of birth is in conflict with the date of death"
                            ...
                            self.assertTrue(
                                any(
                                    err.startswith("The minimum age to be allowed ")
                    ...
                    if violation_type == "too dead":
                        self.assertEqual(
                            ...
                            {
                                "birth_date": [
                                    "Ensure this value is greater than or equal to {}.".format(
                                        max_past
                                    )
                                ],
                            },
        ...
        form = self._init_form(
            {
                ...
                "birth_date": self.faker.date_between(
                    start_date="-16y", end_date=date.today()
                ),
            },
    ...
    def test_invalid_names(self):
        ...
        test_data = (
            ...
            (
                "symbols",
                lambda: rstr.punctuation(2)
                + rstr.punctuation(4, 10, include=rstr.lowercase(4)),
                "provide this data in Latin characters",
            ),
            (
                "digits",
            ...
            ("all caps", lambda: rstr.uppercase(6, 12), "Today is not CapsLock day"),
            (
                "many caps",
    ...
    def test_valid_data(self, profile, profile_tag):
        for dataset_type in ("full", "partial"):
            data = {
                ...
                "gender": self.faker.word()
                if dataset_type == "full" or "in book" in profile_tag
                else None,
                "pronoun": (
                    self.faker.random_element(
                        elements=[ch[0] for ch in PRONOUN_CHOICES]
                    )
                    if dataset_type == "full"
                    else None
                ),
...
class ProfileFormTests(ProfileFormTestingBase, WebTest):
        ...
        for profile, profile_tag in (
            ...
            with self.subTest(condition=profile_tag):
                ...
                self.assertEqual(
                    ...
                    set(self.book_required_fields + [NON_FIELD_ERRORS]),
                )
                for field in self.book_required_fields:
                    with self.subTest(field=field):
                        self.assertEqual(
                ...
                if "complex" in profile_tag:
                    ...
                    assert_message = (
                        "Form error does not include a mention of nr of host's places."
                    )
                    self.assertTrue(
                        any(
                            error == assert_content
                            for error in form.errors[NON_FIELD_ERRORS]
                        ),
                        msg=assert_message,
                    )
    ...
    def test_invalid_birth_date_after_death_date(self):
        ...
        self.assertEqual(
            ...
            {
                "birth_date": [
                    "The indicated date of birth is in conflict with the date of death ({:%Y-%m-%d}).".format(
                        profile.death_date
                    )
                ],
            },
    ...
    def test_invalid_birth_date_for_complex_profile(self):
        ...
        today = date.today()
        try:
            almost_old_enough = today.replace(
                year=today.year - self.config.host_min_age
            )
        except ValueError:
            almost_old_enough = today.replace(
                year=today.year - self.config.host_min_age, day=today.day - 1
            )
        finally:
            almost_old_enough += timedelta(days=1)
        error_message = "The minimum age to be allowed hosting is {}.".format(
            self.config.host_min_age
        )
        hosting_data = (
            self.profile_hosting,
            [
                self.faker.date_between(start_date="-10y", end_date="-1y"),
                almost_old_enough,
            ],
            error_message,
        )
        ...
        try:
            almost_old_enough = today.replace(
                year=today.year - self.config.meet_min_age
            )
        except ValueError:
            almost_old_enough = today.replace(
                year=today.year - self.config.meet_min_age, day=today.day - 1
            )
        finally:
            almost_old_enough += timedelta(days=1)
        error_message = (
            "The minimum age to be allowed meeting with visitors is {}.".format(
                self.config.meet_min_age
            )
        )
        meeting_data = (
            self.profile_meeting,
        ...
        for (profile, profile_tag), dates, assert_content in (
            hosting_data,
            meeting_data,
            host_and_meet_data,
        ):
    ...
    def test_form_submit(self, obj=None):
        for new_email in (Faker().email(), " "):
            page = self.app.get(
                reverse(
                    "profile_email_update",
                    kwargs={"pk": self.user.pk, "slug": self.user.autoslug},
                ),
                user=self.user.user,
            )
