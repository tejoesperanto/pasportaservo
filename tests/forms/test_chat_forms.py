...
class WriteFormTests(AdditionalAsserts, WebTest):
    ...
    def do_test_form_submit(self, recipient, deceased, invalid_email=False):
        page = self.app.get(
            reverse("postman:write"), user=self.sender, headers={"Referer": "/origin"}
        )
...
class ReplyFormTests(AdditionalAsserts, WebTest):
    ...
    def setUpTestData(cls):
        ...
        cls.message, cls.message_other = (
            cls._setup_test_message(u) for u in [rec, rec_other]
        )
    ...
    def test_clean_form(self):
        ...
        for form_class in (CustomReplyForm, CustomQuickReplyForm):
            for originator in (self.sender, self.recipient, self.recipient_other):
                with self.subTest(
                    form_class=form_class.__name__, sender=originator.username
                ):
        ...
        for form_class in (CustomReplyForm, CustomQuickReplyForm):
            for originator in (self.sender, self.recipient, self.recipient_other):
                with self.subTest(
                    ...
                    self.assertFalse(form.is_valid())
                    self.assertEndsWith(
                        form.non_field_errors()[0], "This user has passed away."
                    )
    ...
    def do_test_form_submit(self, orig_message, deceased, invalid_email=False):
        ...
        for i, (view_name, form_class) in enumerate(test_views, start=1):
            with self.subTest(form_class=form_class.__name__):
                ...
                page = self.app.get(
                    reverse(
                        "postman:reply", kwargs={"message_id": reply_to_message_id}
                    ),
                    user=self.sender,
                ...
                if deceased:
                    ...
                    self.assertRedirects(
                        page_result, "/origin", fetch_redirect_response=False
                    )
                    if invalid_email:
                        self.assertEqual(len(mail.outbox), 0)
                    else:
                        ...
                        self.assertEndsWith(
                            mail.outbox[0].subject, page.form["subject"].value
                        )
    ...
    def test_form_submit_for_invalid_email(self):
        self.do_test_form_submit(
            orig_message=self._setup_test_message(
                UserFactory(invalid_email=True, profile=None)
            ),
            deceased=False,
            invalid_email=True,
        )
