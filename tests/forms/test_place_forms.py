...
class PlaceFormTestingBase:
    ...
    def setUpTestData(cls):
        ...
        cls.countries_with_optional_region = set(
            cls.faker.random_elements(
                elements=countries_no_mandatory_region, length=15, unique=True
            )
        ) | {"PL", "DE"} - {
            "GM"
        ...
        cls.expected_fields = {
            ...
            "conditions": (
                "random_elements",
                {"elements": cls.conditions, "unique": True},
            ),
        }
        ...
        cls.complete_place = PlaceFactory(
            country=cls.faker.random_element(
                elements=cls.countries_no_predefined_region
            ),
            available=False,
            tour_guide=False,
            have_a_drink=False,
            state_province=cls.faker.county(),
            postcode=True,
    ...
    def _fake_value(self, field_name, country=None, *, prev_value=None, faker=None):
        ...
        if (
            ...
            and country not in self.countries_no_predefined_region
        ):
            ...
            generator_name, generator_params = islice(
                chain(self.expected_fields[field_name], [{}]), 2
            )
    ...
    def test_labels(self):
        ...
        for country in ("US", "RU", "KR", "PL", "DE"):
            ...
            with self.subTest(
                country=country, label=form.fields["state_province"].label
            ):
                self.assertEqual(
                    form.fields["state_province"].label,
                    SUBREGION_TYPES[
                        COUNTRIES_DATA[country]["administrative_area_type"]
                    ].capitalize(),
                )
                self.assertTrue(
                    hasattr(form.fields["state_province"], "localised_label")
                )
                self.assertEqual(
                    form.fields["state_province"].choices[0][1], "---------"
                )
    ...
    def test_blank_data(self):
        ...
        self.assertFalse(form.is_valid())
        self.assertEqual(
            set(form.errors.keys()), set(self.book_required_fields + [NON_FIELD_ERRORS])
        )
        for field in self.book_required_fields:
            ...
            self.assertTrue(
                any(assert_content in error for error in form.errors[NON_FIELD_ERRORS]),
                msg=assert_message,
            )
    ...
    def test_misconfigured_database(self):
        ...
        with self.assertLogs("PasportaServo.address", level="ERROR") as log:
            self._init_empty_form(
                {"country": country, "state_province": self.faker.word()}
            )
                    ...
                    if region:
                        ...
                        if hasattr(form.fields["state_province"], "localised_label"):
                            expected_error_message = (
                                "the name of the {} must be indicated".format(
                                    form.fields["state_province"].label.lower()
                                )
                            )
                        else:
                            expected_error_message = (
                                "the name of the state or province must be indicated"
                            )
                        self.assertTrue(
                            any(
                                expected_error_message in error
        ...
        form = self._init_empty_form(
            {
                "country": self.faker.random_element(
                    elements=self.countries_no_predefined_region
                ),
            }
        )
        ...
        for country in (
            ...
            with self.subTest(
                ...
                form = self._init_empty_form(
                    {
                        "country": country,
                        "state_province": self._fake_value(
                            "state_province", country=country
                        ),
                    }
                )
                self.assertTrue(form.is_valid(), msg=repr(form.errors))
    ...
    def test_invalid_postcode(self):
        ...
        test_data = [
            ...
            (
                "IL",
                self.faker.pyint(100001, 999998),
            ),  # Six digits are not allowed (only 5 or 7).
            (
                "AZ",
                "AZ {}".format(self.faker.pyint(10000, 999999)),
            ),  # Too long (only 4 digits).
            (
                "AZ",
                "AZ{}".format(self.faker.pyint(1000, 9999)),
            ...
            (
                "LT",
                "LT {}".format(self.faker.pyint(10000, 99999)),
            ),  # Invalid prefix separator.
            ...
            (
                "CA",
                self.faker.pystr_format("K# S# P#"),
            ),  # Correct items, incorrect groups split.
        ]
        ...
        test_data = {
            "country": "AM",
            "postcode": str(self.faker.pyint(100000000001, 2000000000002)),
        }
    ...
    def test_valid_postcode(self):
        ...
        form = self._init_empty_form(
            {"postcode": self.faker.pystr_format("????##????")}
        )
        self.assertFalse(form.is_valid())
        self.assertEqual(form.errors, {"country": ["This field is required."]})
        ...
        for field_names in test_combinations:
            with self.subTest(offer=" / ".join(field_names), age=owner.age):
                ...
                if "available" in field_names:
                    ...
                    error_message = (
                        f"The minimum age to be allowed hosting"
                        f" is {self.config.host_min_age}."
                    )
                else:
                    for field_name in field_names:
                        self.assertIn(field_name, form.errors)
                    error_message = (
                        "The minimum age to be allowed meeting with"
                        f" visitors is {self.config.meet_min_age}."
                    )
    ...
    def test_country_dependent_errors(self):
        form = self._init_form(
            instance=self.complete_place, owner=self.complete_place.profile
        )
        form_data = form.initial.copy()

        new_country = self.faker.random_element(
            elements=[
                c
                for c in countries_with_mandatory_region()
                if COUNTRIES_DATA[c]["postcode_regex"]
            ]
        )
        form_data[
            "country"
        ] = new_country  # Old country was one from a different set by definition.
    ...
    def test_save_change_location_data(self, mock_geocode_city, mock_geocode):
        ...
        if "country" not in form_data:
            # The case of PlaceCreateForm.
            form_data["country"] = self.faker.random_element(
                elements=self.countries_no_predefined_region
            )
        test_point = GeoPoint([-21.932887, 64.150438], srid=SRID)
        test_assertion = AssertionError(
            "geocode was not supposed to be called second time"
        )
        test_config = [
            ([None, test_assertion], None, 0),
            ([self.DummyLocation(None), test_assertion], None, 0),
            (
                [self.DummyLocationWithConfidence(test_point, 1), test_assertion],
                None,
                0,
            ),
            (
                [self.DummyLocationWithConfidence(test_point, 6), test_assertion],
                test_point,
                6,
            ),
        ]
        ...
        for field_name in self.location_fields:
            for field_empty in (False, True):
                ...
                if field_name == "country":
                    ...
                    remaining_countries = self.countries_no_predefined_region - set(
                        [data["country"]]
                    )
                    data[field_name] = self.faker.random_element(
                        elements=remaining_countries
                    )
                    ...
                    data[field_name] = (
                        self._fake_value(
                            field_name, data["country"], prev_value=data.get(field_name)
                        )
                        if not field_empty
                        else None
                    )
                for i, (
                    side_effect,
                    expected_loc,
                    expected_loc_confidence,
                ) in enumerate(test_config, start=1):
                    with self.subTest(
                        field=field_name,
                        ...
                        self.assertEqual(
                            Whereabouts.objects.count(), number_coded_cities
                        )
                        ...
                        self.assertEqual(
                            place.location_confidence, expected_loc_confidence
                        )
    ...
    @patch("hosting.forms.places.geocode")
    @patch("hosting.forms.places.geocode_city")
    def test_save_change_location_data_and_address(
        self, mock_geocode_city, mock_geocode
    ):
        form = self._init_form(
            instance=self.complete_place, owner=self.complete_place.profile
        )  # = GET
        ...
        test_config = [
            ...
            (
                [
                    self.DummyLocation(None),
                    self.DummyLocationWithConfidence(test_point_B, 6),
                ],
                test_point_B,
                6,
            ),
            ...
            (
                [self.DummyLocationWithConfidence(test_point_A, 8), test_assertion],
                test_point_A,
                8,
            ),
        ]
        ...
        for field_name in self.location_fields:
            for field_empty in (False, True):
                ...
                data["address"] = getattr(
                    self.faker, self.expected_fields["address"][0]
                )()
                for i, (
                    side_effect,
                    expected_loc,
    ...
    def test_save_geocode_existing_city(self, mock_geocode_city, mock_geocode):
        ...
        mock_geocode_city.side_effect = AssertionError(
            "geocode-city was unexpectedly called"
        )
        ...
        for region in ("mandatory", "optional", "unrestricted"):
            ...
            form = self._init_form(
                instance=self.simple_place, owner=self.complete_place.profile
            )
            form_data = form.initial.copy()
            ...
            if region == "mandatory":
                ...
                form_data["state_province"] = self._fake_value(
                    "state_province", form_data["country"]
                )
            ...
            for field_empty in (False, True):
                ...
                with self.subTest(
                    ...
                    city_has_value="✓ " if not field_empty else "✗ ",
                ):
                    self.simple_place.refresh_from_db()
                    form = self._init_form(
                        data=form_data,
    ...
    def test_save_geocode_new_city(self, mock_geocode_city, mock_geocode):
        ...
        for region in ("mandatory", "optional", "unrestricted"):
            for field_empty in (False, True):
                ...
                if region == "mandatory":
                    form_data["country"] = self.faker.random_element(
                        elements=countries_with_mandatory_region()
                    )
                elif region == "optional":
                    form_data["country"] = self.faker.random_element(
                        elements=self.countries_with_optional_region
                    )
                else:
                    form_data["country"] = self.faker.random_element(
                        elements=self.countries_no_predefined_region
                ...
                if not field_empty:
                    ...
                    form_data["city"] = "{}_{}".format(
                        self._fake_value("city"), datetime.now().timestamp()
                    )
                    mock_geocode_city.return_value = GeoResult(
                        [35.304816, 32.706630],
                        {
                            "northeast": [35.313535, 32.736300],
                            "southwest": [35.266052, 32.672766],
                        },
                    )
                ...
                with self.subTest(
                    ...
                    if field_empty:
                        ...
                        self.assertEqual(
                            Whereabouts.objects.count(), number_coded_cities + 1
                        )
                        ...
                        if region == "mandatory":
                            self.assertEqual(
                                whereabouts.state, form_data["state_province"]
                            )
                        else:
        ...
        test_data = [
            ...
            (
                Faker().random_elements(
                    elements=self.conditions, length=3, unique=True
                ),
                True,
                "",
            ),
    ...
    def test_form_submit_location_data(self, mock_geocode_city, mock_geocode):
        ...
        altered_place = self._get_altered_place()
        self.assertRedirects(
            page, reverse("place_location_update", kwargs={"pk": altered_place.pk})
        )
        ...
        self.assertEqual(
            altered_place.location_confidence, LocationConfidence.UNDETERMINED
        )
...
class PlaceFormTests(PlaceFormTestingBase, AdditionalAsserts, WebTest):
    ...
    def test_save_no_change(self, mock_geocode_city, mock_geocode):
        ...
        form = self._init_form(
            data=form.initial.copy(), instance=self.simple_place
        )  # = POST
    ...
    def test_save_change_non_location_data(self, mock_geocode_city, mock_geocode):
        ...
        for field_name in (
            ...
            for field_empty in (False, True):
                data = form_data.copy()
                if not field_empty:
                    data[field_name] = self._fake_value(
                        field_name, prev_value=data[field_name]
                    )
                else:
                    data[field_name] = None
                with self.subTest(
                    field=field_name, has_value="✓ " if not field_empty else "✗ "
                ):
                    self.complete_place.refresh_from_db()
                    ...
                    mock_geocode.side_effect = AssertionError(
                        "geocode was unexpectedly called"
                    )
                    mock_geocode_city.side_effect = AssertionError(
                        "geocode-city was unexpectedly called"
                    )
...
class PlaceCreateFormTests(PlaceFormTestingBase, AdditionalAsserts, WebTest):
    ...
    def _get_view_page(self):
        return self.app.get(
            reverse(
                "place_create", kwargs={"profile_pk": self.complete_place.profile.pk}
            ),
    ...
    def _init_page_form_for_submission(self, page, modify_fields, test_data=None):
        data = test_data or {}
        for field_name in (
            set(self.expected_fields) - set(modify_fields) - set(["conditions"])
        ):
            page.form[field_name] = data[field_name] = getattr(
                self.complete_place, field_name
            )
        return super()._init_page_form_for_submission(page, modify_fields, data)

    def _get_altered_place(self):
        return (
            Place.all_objects.filter(owner=self.complete_place.profile)
            .order_by("-id")
            .first()
        )
    ...
    def test_form_submit_non_location_data(self, mock_geocode_city, mock_geocode):
        mock_geocode.side_effect = [
            self.DummyLocationWithConfidence(
                GeoPoint([-19.624239, 63.627832], srid=SRID), 8
            ),
            AssertionError("geocode was not supposed to be called second time"),
        ]

        super().test_form_submit_non_location_data.__wrapped__(
            self, mock_geocode_city, mock_geocode
        )
...
class PlaceLocationFormTests(WebTest):
    ...
    def test_init(self):
        ...
        self.assertIn(
            "data-selectable-zoom", form_empty.fields["location"].widget.attrs
        )
        # Verify that the form's save method is protected in templates.
        self.assertTrue(
            hasattr(form_empty.save, "alters_data")
        ...
        for role in (OWNER, SUPERVISOR):
            with self.subTest(view_role=role):
                ...
                form = PlaceLocationForm(
                    data={"location": ""}, instance=self.place, view_role=role
                )
                self.assertTrue(form.is_valid(), msg=repr(form.errors))
                place = form.save(commit=False)
    ...
    def test_save_value(self):
        for role, expected_confidence in (
            [OWNER, LocationConfidence.EXACT],
            ...
            with self.subTest(view_role=role):
                ...
                self.place.location_confidence = LocationConfidence.LT_25KM
                form = PlaceLocationForm(
                    data={"location": str(self.point_one)},
                    instance=self.place,
                    view_role=role,
                )
                self.assertTrue(form.is_valid(), msg=repr(form.errors))
    ...
    def test_form_submit(self, user=None, conf=LocationConfidence.EXACT):
        ...
        self.place.refresh_from_db()
        self.assertRedirects(
            page, reverse("place_detail_verbose", kwargs={"pk": self.place.pk})
        )
