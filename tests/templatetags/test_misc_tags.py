...
class RandomIdentifierTagTests(TestCase):
    template_string = string.Template(
        "{% load random_identifier from utils %}{% random_identifier $LENGTH %}"
    )
...
class PublicIdFilterTests(TestCase):
    ...
    def test_valid_object(self):
        ...
        obj = Cls(
            pk=1023, name="John", date_joined="2011-02-03", date_deleted="2018-07-06"
        )
        with self.subTest(obj=obj):
            page = Template(self.template_string).render(Context({"my_obj": obj}))
            self.assertEqual(
                page, "d64d289bce1a4d5a355bf948a58af770842a008d74bd375f57d182375838994c"
            )
...
class ListTagTests(TestCase):
    ...
    def test_list_output(self):
        ...
        page = Template(
            "{% load list from utils %}{% list 'aa' +2 'bb' -2 'cc' %}"
        ).render(Context())
...
class DictTagTests(TestCase):
    ...
    @unittest.skipIf(
        sys.version_info < (3, 6), "Order of dict cannot be guaranteed in Py < 3.6"
    )
    def test_dict_output(self):
        # A list of parameters is expected to result in a dict containing those keys and values.
        page = Template(
            "{% load dict from utils %}{% dict aa=True bb=False cc=None %}"
        ).render(Context())
        self.assertEqual(
            page, "{&#39;aa&#39;: True, &#39;bb&#39;: False, &#39;cc&#39;: None}"
        )
    ...
    def test_dict_result(self):
        ...
        expected_value = {
            "on": "[a],[b],[c],;\n{}[1:a=A&lt;a],[2:b=b&gt;B],[3:c=C&amp;c],.".format(
                " " * 16
            ),
...
class CompactFilterTests(TestCase):
    def test_single_line(self):
        ...
        page = Template("{% load compact from utils %}[{{ my_var|compact }}]").render(
            Context({"my_var": content})
        )
    ...
    def test_autoescape(self):
        ...
        for switch in ("on", "off"):
            with self.subTest(autoescape=switch):
                template = Template(
                    template_string.substitute(SWITCH=switch, CONTENT=content)
                )
                page = template.render(Context({"my_var": content}))
                self.assertEqual(
                    page.replace(" " * 16, "").strip(),
                    '[Praesent <nonummy mi> "in odio".]\n'
                    + (escape if switch == "on" else lambda x: x)(
                        '[Praesent <nonummy mi> "in odio".]/'
                    )
                    * 2,
                )
        ...
        for content, expected_values in test_data or self.test_data:
            for sep in expected_values:
                with self.subTest(value=content, separator=sep):
                    ...
                    self.assertEqual(
                        page.strip(),
                        "".join("#{}#".format(part) for part in expected_values[sep]),
                    )
    ...
    def do_test_with_chunks(self, *, var, autoescape):
        ...
        expected = {
            ...
            "<~": [
                ...
                [
                    "This message;\t",
                    "strong>along with the apple",
                    "/strong>; is sent on behalf of ",
                    "span>Adam",
                    "/span>;",
                ],
            ],
            ...
            "\t~\t~42": [
                True,
                [
                    "This message;\t<strong>along with the apple",
                    "</strong>; is sent on behalf of <span>Adam",
                    "</span>;",
                ],
            ],
            ...
            with self.subTest(separator=sep):
                ...
                self.assertEqual(
                    ...
                    "".join(
                        "#{}#".format(
                            escape(part)
                            if autoescape and (var or expected[sep][0])
                            else part
                        )
...
class MultFilterTests(TestCase):
    ...
    test_data = [
        ...
        (
            False,
            {3: 0, -3: 0, "3": 0, "007": 0, "-3": 0, "": "", "0.03": "", object(): ""},
        ),
        (
            None,
            {
    ...
    def test_safe_values(self):
        template_string = string.Template(
            "{% load mult from utils %}{{ $CONTENT|mult:by }}"
        )
                ...
                with self.subTest(value=value, by=multiplier):
                    template = Template(
                        template_string.substitute(
                            ...
                            if value is self.dummy_object
                            else value
                        )
                    )
                    page = template.render(
                        Context({"my_var": self.dummy_object, "by": multiplier})
                    )
            ...
            for multiplier, outcome in tests.items():
                for switch in ("on", "off"):
                    with self.subTest(value=value, by=multiplier, autoescape=switch):
                        template = Template(template_string.substitute(SWITCH=switch))
                        page = template.render(
                            Context({"my_var": value, "by": multiplier})
                        )
                        self.assertEqual(
