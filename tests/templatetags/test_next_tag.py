...
class NextTagTests(TestCase):
    ...
    def test_incorrect_syntax(self):
        ...
        for variant in ["{% next %}", "{% next as url %}"]:
            with self.subTest(faulty_tag=variant):
                ...
                self.assertIn(
                    "did not receive value(s) for the argument(s): 'proceed_to'",
                    str(cm.exception),
                )
        ...
        for variant in [
            "{% next url_only=True %}",
            "{% next default=X %}",
            "{% next url_only=True as url %}",
        ]:
            with self.subTest(faulty_tag=variant):
                with self.assertRaises(TypeError) as cm:
                    Template("{% load next from utils %}" + variant).render(Context())
                self.assertIn(
                    "missing 1 required positional argument: 'proceed_to'",
                    str(cm.exception),
                )
    ...
    def test_provided_link(self):
        ...
        template = Template(
            "{% load next from utils %}{% next '/xx/yy/zz' '#qq-' 32 %}"
        )
        page = template.render(Context())
        self.assertEqual(page, expected_value)

        template = Template(
            "{% load next from utils %}{% next '/xx/yy/zz' '#qq-' obj_pk %}"
        )
    ...
    def test_thispage_link(self):
        ...
        expected_value = "{}=%2Ftrololo%2Flolo%2Flo-lo-lo%3FLoLoLo".format(
    ...
    def test_thispage_shortcut_link(self):
        ...
        expected_value = "{}=%2Ftra%2FLA%2Fla%2Fla-la%23y2".format(
            self.redirect_field_name
        )
    ...
    def test_nextpage_link(self):
        ...
        page = template.render(
            Context({"request": self.request_factory.get("/ensaluti/")})
        )
        self.assertEqual(page, "")

        request = self.request_factory.get(
            "/ensaluti/?{}=/profilo/eduard/khil/1976%23familio".format(
                self.redirect_field_name
            )
        )
        expected_value = "{}=%2Fprofilo%2Feduard%2Fkhil%2F1976%23familio".format(
            self.redirect_field_name
        )
        ...
        template = Template(
            "{% load next from utils %}{% next 'next page' '#2009' '=3M' %}"
        )
        page = template.render(Context({"request": request}))
    ...
    def test_urlonly_value(self):
        ...
        template = Template(
            "{% load next from utils %}[{% next '/abc/def/ghij?klmn=op' url_only=True %}]"
        )
        page = template.render(Context())
    ...
    def test_default_value(self):
        expected_value = "{}=Finally%20Returning%20Back%20Home".format(
            self.redirect_field_name
        )

        # A value provided as 'default' is expected to be used when the first argument evaluates to empty string.
        template = Template(
            "{% load next from utils %}{% next '' default='Finally Returning Back Home' %}"
        )
        page = template.render(Context())
        self.assertEqual(page, expected_value)

        template = Template(
            "{% load next from utils %}{% next next_url default=builtin_url %}"
        )
        page = template.render(Context())
        self.assertEqual(page, "")
        ...
        template = Template(
            "{% load next from utils %}[{% next next_url url_only=True default=builtin_url %}]"
        )
        page = template.render(
        ...
        template = Template(
            "{% load next from utils %}{% next 'next page' default=builtin_url %}"
        )
