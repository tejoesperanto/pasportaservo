...
class DomainTagTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.template_with_url = Template(
            "{% load domain %}{% domain '/418?I=am&Teapot' %}"
        )
    ...
    def test_through_django_email(self):
        ...
        page = self.template_sans_url.render(
            Context({"protocol": "xkcd", "domain": "initialism"})
        )
        self.assertEqual(page, "xkcd://initialism")
        page = self.template_with_url.render(
            Context({"protocol": "xkcd", "domain": "initialism"})
        )
    ...
    def test_through_postman_email(self):
        ...
        page = self.template_sans_url.render(
            Context({"site": Site.objects.get(name="MyTestServer")})
        )
        self.assertEqual(page, "http://mytestsrv")
        page = self.template_with_url.render(
            Context({"site": Site.objects.get(name="MyTestServer")})
        ...
        page = self.template_sans_url.render(
            Context({"site": Site.objects.get(name="TestPS")})
        )
        ...
        self.assertEqual(
            page, "https://test.pasportaservo.org.mondo/418?I=am&amp;Teapot"
        )
