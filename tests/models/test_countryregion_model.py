...
class CountryRegionModelTests(TestCase):
    ...
    def test_translated_or_latin_name(self):
        ...
        region_without_eo_with_code = CountryRegionFactory.build(
            short_code=True, esperanto_name=""
        )
        region_without_eo_with_name = CountryRegionFactory.build(
            short_code=False, esperanto_name=""
        )

        with override_settings(LANGUAGE_CODE="eo"):
            # For region with an Esperanto name, value is expected to be that name for Esperanto locale.
            self.assertEqual(
                region_with_eo_and_code.translated_or_latin_name,
                region_with_eo_and_code.esperanto_name,
            )
            self.assertEqual(
                region_with_eo_and_name.translated_or_latin_name,
                region_with_eo_and_name.esperanto_name,
            )
            # For region with no Esperanto name,
            # value is expected to be the latin name or code for Esperanto locale.
            self.assertEqual(
                region_without_eo_with_code.translated_or_latin_name,
                region_without_eo_with_code.latin_name,
            )
            self.assertEqual(
                region_without_eo_with_name.translated_or_latin_name,
                region_without_eo_with_name.latin_code,
            )
        with override_settings(LANGUAGE_CODE="en"):
            ...
            self.assertEqual(
                region_with_eo_and_code.translated_or_latin_name,
                region_with_eo_and_code.latin_name,
            )
            self.assertEqual(
                region_with_eo_and_name.translated_or_latin_name,
                region_with_eo_and_name.latin_code,
            )
            # For region with no Esperanto name,
            # value is expected to be the latin name or code for non-Esperanto locale.
            self.assertEqual(
    ...
    def test_display_value(self):
        test_data = [
            ...
            (
                ...
                dict(
                    latin_code="Neighbourhood of Bavdhan",
                    latin_name="",
                    local_code="Bavdhan",
                ),
                "Bavdhan",
            ...
            (
                ...
                # value is expected to be only the local code.
                dict(
                    latin_code="Bavdhan", latin_name="", local_code="Bavdhan Locality"
                ),
                "Bavdhan Locality",
            ),
            (
            ...
            (
                ...
                # value is expected to be both latin code with the local code.
                dict(
                    latin_code="Bavdhan", latin_name="", local_code="Bāvdhān Locality"
                ),
        ...
        for kwargs, expected_value in test_data:
            for esperanto_on, esperanto_name in product(
                (False, True), ("", "Najbarejo", "Bavdhan")
            ):
                with self.subTest(
                    **kwargs, esperanto=esperanto_name, include_esperanto=esperanto_on
                ):
                    region = CountryRegionFactory.build(
                        **kwargs, esperanto_name=esperanto_name
                    )
                    self.assertEqual(
                        region.get_display_value(with_esperanto=esperanto_on),
                        f"{esperanto_name} \xa0\u2013\xa0 {expected_value}"
                        if esperanto_on and esperanto_name
                        else expected_value,
                    )
        ...
        with patch(
            "hosting.models.CountryRegion.latin_code", new_callable=PropertyMock
        ) as mock_name:
    ...
    def test_str(self):
        region = CountryRegionFactory.build(short_code=False)
        self.assertEqual(str(region), f"{region.country.code}: {region.latin_code}")
