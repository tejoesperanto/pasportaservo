...
class TrackingManagersTests:
    ...
    @patch(
        "hosting.managers.SiteConfiguration.get_solo",
        return_value=namedtuple("DummyConfig", "confirmation_validity_period")(
            timedelta(days=35)
        ),
    )
    def test_manager_flags(self, mock_config):
        ...
        with self.subTest(manager="objects"):
            ...
            for obj in qs:
                ...
                self.assertFalse(
                    obj.deleted, msg=f"object deleted on = {obj.deleted_on}"
                )
