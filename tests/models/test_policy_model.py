...
class PolicyModelTests(TestCase):
    ...
    def test_str(self):
        ...
        for consent in True, False:
            policy = PolicyFactory.create(requires_consent=consent)
            for lang in expected_strings:
                with (
                    override_settings(LANGUAGE_CODE=lang),
                    self.subTest(consent=consent, lang=lang)
                ):
                    self.assertEqual(
                        str(policy),
...
class PoliciesManagerTests(AdditionalAsserts, TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.effective_policies = [
            sorted(policies, key=lambda p: p.effective_date)[-1],
        ]
        ...
        cls.effective_policies.extend(
            sorted(policies, key=lambda p: p.effective_date)
        )
    ...
    def test_get_all_effective_today(self):
        ...
        self.assertEqual(
            policies[0].version,
            self.effective_policies[-1].version)
        self.assertEqual(
