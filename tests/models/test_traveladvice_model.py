...
class TravelAdviceModelTests(AdditionalAsserts, WebTest):
    ...
    def test_get_for_country(self):
        ...
        for country, active, expected in (
            ("BB", True, []),
            ("BB", False, []),
            ...
            with self.subTest(country=country, is_active=active):
                self.assertQuerysetEqual(
                    TravelAdvice.get_for_country(country, active),
                    expected,
                    transform=_tag,
                )
        with self.subTest(country="AA", is_active=None):
            self.assertQuerysetEqual(
                TravelAdvice.get_for_country("AA", None),
                tags.values(),
                ordered=False,
                transform=_tag,
            )
    ...
    def test_active_status_flag(self):
        ...
        test_data = [
            ...
            (
                ...
                False,
                TravelAdviceFactory(
                    active_from=None, active_until=faker.date_between("-30d", "-2d")
                ),
            ),
            (
                "in future",
            ...
            (
                ...
                False,
                TravelAdviceFactory(
                    active_from=faker.date_between("+2d", "+30d"), active_until=None
                ),
            ),
            (
                "in present",
            ...
            (
                ...
                True,
                TravelAdviceFactory(
                    active_from=faker.date_between("-30d", "today"), active_until=None
                ),
            ),
            (
                "in present",
                True,
                TravelAdviceFactory(
                    active_from=None, active_until=faker.date_between("today", "+30d")
                ),
            ),
            (
                "in present",
