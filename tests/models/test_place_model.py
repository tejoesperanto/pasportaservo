...
class PlaceModelTests(AdditionalAsserts, TrackingManagersTests, WebTest):
    ...
    def test_is_blocked(self):
        ...
        self.assertTrue(place.is_blocked)
        place = PlaceFactory.build(
            blocked_from=Faker("future_date"), blocked_until=None
        )
        self.assertTrue(place.is_blocked)
        ...
        self.assertTrue(place.is_blocked)
        place = PlaceFactory.build(
            blocked_from=None, blocked_until=Faker("future_date")
        )
        self.assertTrue(place.is_blocked)
        ...
        place = PlaceFactory.build(
            blocked_from=date.today(), blocked_until=date.today()
        )
        self.assertTrue(place.is_blocked)
        place = PlaceFactory.build(
            blocked_from=date.today(), blocked_until=Faker("future_date")
        )
        self.assertTrue(place.is_blocked)
        place = PlaceFactory.build(
    ...
    def test_absolute_url(self):
        place = self.basic_place
        self.assertEqual(place.get_absolute_url(), "/ejo/{}/".format(place.pk))
