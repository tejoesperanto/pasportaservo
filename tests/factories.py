...
class PolicyFactory(DjangoModelFactory):
    ...
    def content(obj, n):
        ...
        policy_date = (
            faker.date(pattern="%Y-%m-%d") if obj.from_date is None else obj.from_date
        )
...
class ProfileFactory(DjangoModelFactory):
    ...
    generated_name = LocaleFaker(
        "pystr_format", string_format="{{first_name}}//{{last_name}}"
    )
    ...
    death_date = factory.Maybe(
        "deceased", yes_declaration=Faker("date_this_decade"), no_declaration=None
    )
    description = Faker("paragraph", nb_sentences=4)
    email = factory.Maybe(
        "with_email", yes_declaration=Faker("email"), no_declaration=""
    )
    ...
    def number(self):
        ...
        while not phone.is_valid():
            phone = PhoneNumber(
                ...
                national_number=randint(10000000, 9999999990),
            )
        return phone
...
class GenderFactory(DjangoModelFactory):
    ...
    id = factory.Sequence(
        lambda n: GenderFactory._meta.get_model_class()
        .objects.values("id")
        .last()["id"]
        + n
        + 1
...
class CountryRegionFactory(DjangoModelFactory):
    ...
    country = factory.LazyFunction(lambda: Country(choice(list(COUNTRIES))))
    iso_code = Faker(
        "pystr_format", string_format="???#", letters="ABCDEFGHJKLMNPQRSTUVWXYZ"
    )
    ...
    latin_name = factory.Maybe(
        "short_code", yes_declaration=Faker("sentence", nb_words=3), no_declaration=""
    )
...
class WhereaboutsFactory(DjangoModelFactory):
    ...
    @factory.lazy_attribute
    def bbox(self):
        minx, miny, maxx, maxy = self.center.buffer(
            width=uniform_random(0.05, 0.45), quadsegs=2
        ).extent
...
class TravelAdviceFactory(DjangoModelFactory):
    ...
    def active_from(self):
        ...
        if self.in_past:
            faked_date = (
                faker.date_between(start_date="-365d", end_date="-200d")
                if random() < 0.85
                else None
            )
        elif self.in_future:
            ...
            faked_date = (
                faker.date_between(start_date="-200d", end_date="-2d")
                if random() < 0.85
                else None
            )
        else:
            faked_date = (
                faker.date_object(end_datetime="+5y") if random() < 0.85 else None
            )
        return faked_date
    ...
    def active_until(self):
        ...
        if self.in_past:
            ...
            faked_date = (
                faker.date_between(start_date="+200d", end_date="+365d")
                if random() < 0.85
                else None
            )
        elif self.in_present:
            faked_date = (
                faker.date_between(start_date="+2d", end_date="+200d")
                if random() < 0.85
                else None
            )
        else:
            if self.active_from:
                ...
                faked_date = (
                    faker.date_between_dates(date_start=start, date_end=end)
                    if random() < 0.85
                    else None
                )
            else:
